...
| `scripts/research_regime_v2_evaluator.py` | Research-only Regime v2 scorecard/evaluator using strategy-utility labels, route-outcome equity curves, false-bull/rebound guardrails, no-lookahead selector route, route robustness gates, optional label/route-objective tuning, and failure diagnostics | `python3 scripts/research_regime_v2_evaluator.py --days 30 --coins SOL,SUI,AAVE,LINK,AVAX,JUP --step-hours 6 --forward-hours 24 --tune-scorecard --tune-route-objective --selector-lookback 12 --output /tmp/regime_v2.json` |
| `scripts/regime_promotion_readiness.py` | Report-only Regime v2 readiness summary across 30d/60d/90d artifacts; emits 🟢/🟡/🔴 verdict and blockers; never promotes automatically | `python3 scripts/regime_promotion_readiness.py --fresh --days 30,60,90 --json-output /tmp/regime_readiness.json` |
| `scripts/regime_v2_forward_replay.py` | Cached forward-history replay harness: fetches candles once, reuses them for many Regime v2 settings/windows, and ranks candidates without lookahead | `python3 scripts/regime_v2_forward_replay.py --days 30,60,90 --step-hours 12 --selector-lookbacks 6,12,24 --output /tmp/regime_v2_forward_replay.json` |
